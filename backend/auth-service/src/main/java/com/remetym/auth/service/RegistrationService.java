...
public class RegistrationService {
    ...
    public RegistrationRequest register(RegisterRequest req) {
        ...
        Optional<RegistrationRequest> existingReq = registrationRequestRepository.findByEmailIgnoreCase(cleanEmail);
        ...
        request.setTitle("DHO".equals(cleanRole) ? "District Health Officer" : "PHC Staff");
        ...
        // Save pending record in User table as well
    ...
    private void sendAdminRegistrationNotification(RegistrationRequest request) {
        try {
            String facility = "PHC_STAFF".equalsIgnoreCase(request.getRole()) && request.getPhcName() != null ? request.getPhcName() : request.getDistrictName();
            String title = "New " + ("DHO".equalsIgnoreCase(request.getRole()) ? "DHO" : "PHC Staff") + " Registration";
            String message = "Applicant " + request.getFullName() + " (" + request.getEmail() + ") requested " + request.getRole() + " access for " + facility + ". Status: PENDING.";
            ...
            String jsonPayload = String.format(
                ...
                escapeJson(title),
                escapeJson(message),
                request.getRegisteredAt() != null ? request.getRegisteredAt() : Instant.now().toString()
            );
            ...
            java.net.http.HttpRequest httpRequest = java.net.http.HttpRequest.newBuilder()
                .uri(java.net.URI.create(gatewayUrl + "/api/notifications"))
                .header("Content-Type", "application/json")
                .POST(java.net.http.HttpRequest.BodyPublishers.ofString(jsonPayload))
                .build();

            client.sendAsync(httpRequest, java.net.http.HttpResponse.BodyHandlers.ofString())
                .thenAccept(res -> System.out.println("[ADMIN NOTIFICATION DISPATCHED] Status: " + res.statusCode()))
                .exceptionally(err -> {
                    System.out.println("[ADMIN NOTIFICATION NOTICE] Async notification send skipped: " + err.getMessage());
                    return null;
                });
        } catch (Exception e) {
    ...
    public RegistrationRequest approveRequest(String requestId, String adminName) {
        ...
        req.setApprovedBy(adminName != null ? adminName : "System Admin");
        ...
        user.setDistrictId(req.getDistrictId() != null ? req.getDistrictId() : DistrictIdGenerator.generateDistrictId(req.getDistrictName()));
        ...
        // Auto-sync approved PHC into MongoDB phcs collection
        if (req.getPhcName() != null && !req.getPhcName().trim().isEmpty()) {
            syncPhcToMedicineService(req.getPhcId(), req.getPhcName(), req.getDistrictId(), req.getDistrictName(), req.getEmail());
        }

        // Send REAL Approval Email
        String facility = req.getPhcName() != null ? req.getPhcName() : req.getDistrictName();
        emailService.sendApprovalEmail(req.getEmail(), req.getFullName(), req.getRole(), facility);

        return req;
    }

    private void syncPhcToMedicineService(String phcId, String phcName, String districtId, String districtName, String email) {
        if (phcName == null || phcName.trim().isEmpty()) return;
        try {
            String jsonPayload = String.format(
                "{\"phcId\":\"%s\",\"phcName\":\"%s\",\"districtId\":\"%s\",\"districtName\":\"%s\",\"email\":\"%s\"}",
                escapeJson(phcId != null ? phcId : "PHC-" + System.currentTimeMillis()),
                escapeJson(phcName),
                escapeJson(districtId != null ? districtId : DistrictIdGenerator.generateDistrictId(districtName)),
                escapeJson(districtName != null ? districtName : ""),
            ...
            java.net.http.HttpClient client = java.net.http.HttpClient.newHttpClient();
            java.net.http.HttpRequest httpRequest = java.net.http.HttpRequest.newBuilder()
                .uri(java.net.URI.create(gatewayUrl + "/api/phcs"))
                .header("Content-Type", "application/json")
                .POST(java.net.http.HttpRequest.BodyPublishers.ofString(jsonPayload))
                .build();

            client.sendAsync(httpRequest, java.net.http.HttpResponse.BodyHandlers.ofString())
                .thenAccept(res -> System.out.println("[PHC SYNC DISPATCHED] Status: " + res.statusCode()))
                .exceptionally(err -> {
                    System.out.println("[PHC SYNC NOTICE] Async send skipped: " + err.getMessage());
                    return null;
                });
        } catch (Exception e) {
            System.err.println("[PHC SYNC ERROR]: " + e.getMessage());
        }
    }

    public RegistrationRequest rejectRequest(String requestId, String adminName, String reason) {
        RegistrationRequest req = registrationRequestRepository.findById(requestId)
                .orElseThrow(() -> new RuntimeException("Registration request not found."));

        req.setStatus("REJECTED");
        req.setRejectedAt(Instant.now().toString());
        req.setRejectedBy(adminName != null ? adminName : "System Admin");
        ...
        String facility = req.getPhcName() != null ? req.getPhcName() : req.getDistrictName();
        emailService.sendRejectionEmail(req.getEmail(), req.getFullName(), req.getRole(), facility, reason);

        return req;
    }

    public RegistrationRequest deactivateUser(String identifier, String adminName) {
        Optional<RegistrationRequest> reqOpt = registrationRequestRepository.findById(identifier);
        if (reqOpt.isEmpty()) {
            reqOpt = registrationRequestRepository.findByEmailIgnoreCase(identifier);
        }
        ...
        String userEmail = userOpt.isPresent() ? userOpt.get().getEmail() : reqOpt.get().getEmail();
        String userName = userOpt.isPresent() ? userOpt.get().getName() : reqOpt.get().getFullName();

        if (userOpt.isPresent()) {
            User user = userOpt.get();
            if (!"ACTIVE".equalsIgnoreCase(user.getStatus()) && !"APPROVED".equalsIgnoreCase(user.getStatus())) {
                throw new RuntimeException("Only currently APPROVED users can be deactivated.");
            }
        ...
        try {
            emailService.sendDeactivationEmail(userEmail, userName);
        } catch (Exception e) {
            System.err.println("[DEACTIVATION EMAIL NOTICE]: " + e.getMessage());
        }

        return req;
    }

    public RegistrationRequest reactivateUser(String identifier, String adminName) {
        Optional<RegistrationRequest> reqOpt = registrationRequestRepository.findById(identifier);
        if (reqOpt.isEmpty()) {
            reqOpt = registrationRequestRepository.findByEmailIgnoreCase(identifier);
        }
        ...
        if (userOpt.isPresent()) {
            ...
            if (!"DEACTIVATED".equalsIgnoreCase(user.getStatus())) {
                throw new RuntimeException("Only DEACTIVATED users can be reactivated.");
            }
            user.setStatus("ACTIVE");
            userRepository.save(user);
        }
        ...
        try {
            emailService.sendReactivationEmail(userEmail, userName);
        } catch (Exception e) {
            System.err.println("[REACTIVATION EMAIL NOTICE]: " + e.getMessage());
        }
    ...
    public java.util.Map<String, Object> getOracleStats() {
        long totalPhcs = userRepository.findAll().stream()
                .filter(u -> "PHC_STAFF".equalsIgnoreCase(u.getRole()) && "ACTIVE".equalsIgnoreCase(u.getStatus()))
                .map(u -> u.getPhcName() != null ? u.getPhcName().trim().toLowerCase() : u.getPhcId())
                .filter(name -> name != null && !name.isEmpty())
                .distinct()
                .count();

        long approvedPhcReqs = registrationRequestRepository.findByStatus("ACTIVE").stream()
                .filter(r -> "PHC_STAFF".equalsIgnoreCase(r.getRole()) && r.getPhcName() != null)
                .map(r -> r.getPhcName().trim().toLowerCase())
                .distinct()
                .count();
        totalPhcs = Math.max(totalPhcs, approvedPhcReqs);

        long dhoUserCount = userRepository.findAll().stream()
                .filter(u -> "DHO".equalsIgnoreCase(u.getRole()) && "ACTIVE".equalsIgnoreCase(u.getStatus()))
                .count();

        long totalDistricts = userRepository.findAll().stream()
                .filter(u -> "ACTIVE".equalsIgnoreCase(u.getStatus()) && u.getDistrictName() != null && !u.getDistrictName().trim().isEmpty())
                .map(u -> u.getDistrictName().trim().toLowerCase())
                .distinct()
                .count();

        long pendingReqs = registrationRequestRepository.countByStatus("PENDING");
        long approvedReqs = registrationRequestRepository.countByStatus("ACTIVE");
        long rejectedReqs = registrationRequestRepository.countByStatus("REJECTED");
