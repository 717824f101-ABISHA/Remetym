...
def fetch_live_or_generate_historical_data():
    """
    Fetches real historical consumption records from backend database
    ...
    try:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                for item in data:
                    records.append({
                        ...
                        'medicineName': item.get(
                            'medicineName',
                            'Medicine'
                        ),
                        'quantity_dispensed': int(
                            item.get('quantityConsumed', 15)
                        )
                    })
                ...
                print(
                    f"[XGBoost Training] Loaded {len(df)} "
                    "real historical consumption records from backend API."
                )
        ...
        print(
            f"[XGBoost Training WARNING] Could not fetch live historical "
            f"dataset ({e}), constructing baseline dataset."
        )
    ...
    medicines = [
        ...
        {
            'id': 'MED-004',
            'name': 'Amoxicillin Oral Suspension 125mg',
            'base_demand': 15
        },
        {
            'id': 'MED-005',
        ...
        for med in medicines:
            ...
            for d in dates:
                ...
                day_factor = (
                    ...
                    else (0.8 if day_of_week == 6 else 1.0)
                )
                ...
                demand = max(
                    ...
                    int(
                        round(
                            base * day_factor * month_factor + noise
                        )
                    )
                )
...
def train_xgboost_model():
    ...
    # 80% Train, 20% Test - NO random shuffle
    ...
    #
    # IMPORTANT:
    # Do NOT save the pandas historical_df here.
    # Saving it caused pandas StringDtype compatibility
    # problems when loading the artifact on Render.
    os.makedirs(
    ...
    print(
        f"XGBoost Model successfully saved to: {MODEL_PATH}"
    )
